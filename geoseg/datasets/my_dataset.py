...
def val_aug(img, mask):
    img, mask = np.array(img), np.array(mask)
    print("mg.shape, mask.shape",img.shape, mask.shape)
...
class MyDataSet(Dataset):
    def __init__(self, data_root='c', mode='val', img_dir='TrainingImage', mask_dir='TrainingLabel',
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        assert len(img_filename_list) == a
    ...
    def load_img_and_mask(self, index,i=0):
            ...
            img_name = osp.join(self.data_root, self.img_dir, img_id +'_B0'+ self.img_suffix)
            my_int = int(img_id)
