...
class SimVP_Model(nn.Module):
 ...
 ############This section is dedicated to extracting features of the "VV" part. begin#####################
        ...
        for i in  range(4):#(skip1[i]+skip2[i])/2
            skip.append(torch.cat((skip1[i], skip2[i]), dim=1) )
        hid = self.hid(z)
        BS1,C1,H1,W1 = skip[3].shape
      #  print(skip[3].shape)

        hid1 = skip[3].reshape(B,13,C1,H1,W1)
        skip[3] = self.hid1(hid1).reshape(BS1,int(C1/2),H1,W1 )
...
class Decoder(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        ...
        self.dec = nn.Sequential(  
            ConvMYP(int(2 * C_hid), int(C_hid), spatio_kernel, upsampling=samplings[0], act_inplace=act_inplace),  
            *[ConvMYP(3 * C_hid, C_hid, spatio_kernel, upsampling=s, act_inplace=act_inplace) for s in samplings[1:-1]],  
            ConvMYP(3 * C_hid, 4, spatio_kernel, upsampling=samplings[-1], act_inplace=act_inplace)  
        )
    ...
    def forward(self, hid, bs,enc1=None):
        for i in range(0, len(self.dec)):
         #   print(1,hid.shape,enc1[3-i].shape)
          #  print(i,hid.shape,enc1[3-i].shape)
...
class MidMetaNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid, N2,
        ...
        enc_layers = [MetaBlock(
            2*channel_in, channel_hid, input_resolution, model_type,
