...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=8, N_T=8, model_type='gSTA',
        ...
        self.dec = Decoder(hid_S, 4, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, x_raw1, **kwargs):
...
class LSTMUNet(nn.Module):
    def __init__(self, sequence_length, in_channels, img_height, img_width, n_label, lstm_hidden_size=64):
        ...
        #config = ModelConfig()\
        # 假设的输入形状  
        input_shape = (13, 6, 256, 256)  # (batch_size, channels, height, width)  
  
        # 创建模型实例  
        self.lstm  = SimVP_Model(input_shape, hid_hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA')  
    ...
    def forward(self, x):
        ...
        BS =  x.shape[0]  
      #  print(x.shape)
        stacked_tensor = torch.empty(13,BS,6, 256, 256)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu' )
        ...
        for i in range(13):
           ...
           #  n1 = n2/2+n1/2
            ...
            # unet_out = unet_out1/2+unet_out2/2
             unet_out  = torch.cat((n1, n2), dim=1) 
