...
class Xception(nn.Module):
    ...
    def __init__(self, downsample_factor=8):
        ...
        self.conv1 = nn.Conv2d(3*13, 32, 3, 2, 1, bias=False)
...
class LSTMUNet(nn.Module):
    def __init__(self, sequence_length, in_channels, img_height, img_width, n_label, lstm_hidden_size=64):
        ...
        # 创建模型实例  
        self.lstm1  = DeepLab(4)  
        self.lstm2  = DeepLab(4)  
        #self.lstm2  = SimVP_Model(input_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA') 
    ...
    def forward(self, x):
      ...
      #  print(x.shape)
        ...
        stacked_tensor1 = stacked_tensor1.reshape(BS,13*3,256,256)
        stacked_tensor2 = stacked_tensor2.permute(1,0,2,3,4)
       #stacked_tensor2 = stacked_tensor2.permute(1,0,2,3,4)
        stacked_tensor2 = stacked_tensor2.reshape(BS,13*3,256,256)
        lstm_out1 = self.lstm1(stacked_tensor1)
        lstm_out2 = self.lstm1(stacked_tensor2)
        lstm_out1 = (lstm_out1+lstm_out2)/2
      #  lstm_out2 = self.lstm2(stacked_tensor2)
       # lstm1 = self.FinalConv1(lstm_out1)
        #show(lstm_out1,save_path="a")
       # show(lstm1,"InAfter","/data/pth/liangjiaxuan-pth/result366EndAfter.png")
        return     lstm_out1      
