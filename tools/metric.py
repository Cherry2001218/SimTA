...
class Evaluator(object):
    ...
    def add_batch(self, gt_image, pre_image):
        ...
        if gt_image.shape != pre_image.shape:
           reshaped_image = gt_image.reshape(256, 256) 
           self.confusion_matrix += self._generate_matrix(reshaped_image, pre_image)
        else:
    ...
    def under_segmentation_rate(self):#FN / (TP + FN) 
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        IoU = fn / (tp + fn )
        return IoU

    def over_segmentation_rate(self):#FFP / (TP + FP)) 
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        IoU = fp / (tp + fp )
        return IoU
